...
for factor in factors:
    ...
    for hemi in hemis:
        ...
        for view in ["lateral", "medial", "ventral"]:
            ...
            cbar.set_label(r'$p$-values (FDR-corrected)', fontsize=14, labelpad=10)
            cbar.ax.tick_params(labelsize=12)

            # save first
            path = RSA_DIR / f"{condition}_{view}_{hemi}_{factor}_surface.pdf"
    ...
    if factor == 'grade':
        ...
        for hemi in hemis:
            ...
            for roi_name, beta in rois.items():
                ...
                # customize axis 
                ax.set_ylabel(r"$r$ (Fisher's z)", fontsize=15)
                ...
                path = RSA_DIR / f"{condition}_{hemi}_{clean_roi}_grade.pdf"
                fig.savefig(path, format='pdf', transparent=True, bbox_inches='tight')
        ...
        for hemi in hemis:
            ...
            for roi_name, beta in rois.items():
                ...
                g.ax_joint.set_xlabel("Mean accuracy (%)", fontsize=15)
                g.ax_joint.set_ylabel(r"$r$ (Fisher's z)", fontsize=15)
                y_min, y_max = df['Fisher_Z'].min(), df['Fisher_Z'].max()
                ...
                clean_roi = str(roi_name).replace(" ", "_").replace("/", "-")
                path = RSA_DIR / f"{condition}_{hemi}_{clean_roi}_acc.pdf"
                g.savefig(path, format='pdf', transparent=True, bbox_inches='tight')       
